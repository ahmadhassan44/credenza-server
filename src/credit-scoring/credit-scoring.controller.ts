import { Controller, Get, Post, Param, Body, HttpException, HttpStatus, Logger, UseGuards, Request, ForbiddenException } from '@nestjs/common';
...
export class CreditScoringController {
  ...
  @Roles('CREATOR', 'ADMIN')
  async generateScore(@Param('creatorId') creatorId: string, @Request() req): Promise<CreditScore> {
    try {
      // Check if the user has permission to generate a score for this creator
      if (req.user.role !== 'ADMIN' && req.user.creatorId !== creatorId) {
        throw new ForbiddenException('You can only generate credit scores for your own creator account');
      }
  ...
  @Roles('CREATOR', 'ADMIN')
  async getLatestScore(@Param('creatorId') creatorId: string, @Request() req): Promise<CreditScore> {
    try {
      // Check if the user has permission to view scores for this creator
      if (req.user.role !== 'ADMIN' && req.user.creatorId !== creatorId) {
        throw new ForbiddenException('You can only view credit scores for your own creator account');
      }
      
      const score = await this.creditScoringService.getCreatorLatestScore(creatorId);
  ...
  @Roles('CREATOR', 'ADMIN')
  async getScoreHistory(@Param('creatorId') creatorId: string, @Request() req): Promise<CreditScore[]> {
    try {
      // Check if the user has permission to view score history for this creator
      if (req.user.role !== 'ADMIN' && req.user.creatorId !== creatorId) {
        throw new ForbiddenException('You can only view credit score history for your own creator account');
      }
      
      const history = await this.creditScoringService.getCreatorScoreHistory(creatorId);
